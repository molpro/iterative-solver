/*!
 * @file
 * @brief Explicit instantiation of the subspace helper routines in extended precision.
 *
 * LAPACK has no kernels for long double, so this translation unit also serves as a compile-time
 * check that the dense linear algebra of the subspace solvers really is precision-generic and
 * resolves to Eigen for a scalar type LAPACK does not cover.
 */
