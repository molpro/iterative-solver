...
struct LinearEigensystemF : ::testing::Test {
    ...
    std::tuple<std::map<double, std::vector<double>>, std::vector<double>> solve_full_problem(bool hermitian) {
        ...
        molpro::linalg::itsolv::eigenproblem(eigenvector, expected_eigenvalues, hmat_row, metric, n, hermitian, 1.0e-14,
                                             0);
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
        ...
        for (int nroot = 1; nroot <= n && nroot <= 28; nroot += std::max(size_t{1}, n / 10)) {
            for (auto np = 0; np <= n && np <= 100 && (hermitian or np == 0); np += std::max(nroot, int(n) / 5)) {
                molpro::cout << "\n\n*** " << title << ", " << nroot << " roots, problem dimension " << n
                             ...
                             << std::endl;
#ifndef NOFORTRAN
                ASSERT_NE(test_lineareigensystemf(hmat.data(), n, np, nroot, hermitian, expected_eigenvalues.data()),
                          0);
#endif
                auto
                ...
                for (const auto &ee : expected_eigensolutions) {
                    ...
                    auto overlap_with_reference = std::inner_product(parameters.at(root).begin(),
                                                                     parameters.at(root).end(),
                                                                     ee.second.begin(), 0., std::plus<double>(),
